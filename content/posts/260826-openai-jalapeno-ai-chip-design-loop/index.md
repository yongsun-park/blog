...
description = 'OpenAI가 Jalapeño 칩을 개발하며 AI와 설계자가 측정·검증·수정을 반복한 방법과 DeepSeek MLA 커널 최적화 사례를 살펴봅니다.'
...
## OpenAI가 DeepSeek MLA 커널을 최적화했습니다

가장 재미있었던 장표는 `AI turns functional kernels into high-performance implementations`였습니다.

출발점은 기능적으로 맞는 커널과 실행 가능한 테스트입니다. OpenAI 내부 AI 시스템은 칩이나 시뮬레이터에서 결과를 확인하며 구현을 반복해서 바꿉니다. 마지막에는 최적화된 커널을 실제 칩에서 end-to-end로 검증합니다.

장표에는 Jalapeño에서 DeepSeek MLA workload를 실행하기 위한 커널을 최적화한 그래프가 나옵니다. DeepSeek가 이 작업을 했다는 뜻은 아닙니다. OpenAI 내부 AI 시스템이 DeepSeek의 MLA 연산을 위한 Jalapeño 커널을 최적화한 사례입니다.
...
이 사례가 흥미로운 이유는 최적화 대상이 OpenAI 모델만은 아니기 때문입니다. OpenAI 공식 보고서에 따르면 Jalapeño의 원래 생산 계획에 포함되지 않았던 GPT‑OSS, DeepSeek R1, Kimi K2.5도 두 달 안에 높은 성능으로 구동할 수 있게 만들었습니다.

새로운 모델 구조가 등장할 때마다 사람이 커널을 처음부터 최적화하는 대신, 기능적으로 맞는 구현에서 출발해 AI가 실행 결과를 보며 성능을 끌어올린 것입니다. Jalapeño의 경쟁력은 특정 모델에 맞춘 회로뿐 아니라 새로운 모델에 빠르게 적응하는 프로그래밍·검증 환경에도 있어 보입니다.

## OpenAI가 공개한 개선 결과

OpenAI는 최적화된 인간 설계를 기준으로 선택된 연산 블록의 개선 결과를 공개했습니다.
...
OpenAI는 RTL 동결 당일까지 큰 변경을 반영할 수 있었다고 설명합니다. 공식 보고서에서는 선택된 GPT‑OSS attention·MoE 블록의 AI 생성 구현이 기존 전문가 구현보다 1.5~1.8배 빨랐다고 밝혔습니다. 이 수치는 칩 전체나 모델 전체가 아니라 선택된 블록의 결과입니다.
...
발표에서 확인할 수 있는 범위는 분명합니다. AI는 구현 후보를 탐색하고 산술 회로와 모델별 커널을 최적화했습니다. 설계·측정·검증을 반복하는 시간도 줄였습니다.

## 제가 이 발표에서 주목한 부분

이 발표를 보며 좋은 AI 모델만 준비한다고 설계 자동화가 완성되는 것은 아니라는 생각이 들었습니다.

AI가 사용할 수 있는 설계 표현과 실행 도구가 필요합니다. 결과를 비교할 측정값과 잘못된 설계를 걸러 낼 검증 기준도 있어야 합니다. 그래야 Agent가 구현을 조금 바꾸고, 도구를 실행하고, 결과를 확인한 뒤 다음 변경을 결정할 수 있습니다.
